...
def save_checkpoint_with_freq(state, is_best, epoch, save_freq, filepath):
    filename = os.path.join(filepath, 'checkpoint.pth.tar')
    # Save model
    torch.save(state, filename)
...
def calculate_layer_ratio(gradout_list, grad_iter_abs):
    ...
    # print("sum_grad_iter_abs: %s, sum_grad_epoch_abs: %s" % (sum_grad_iter_abs, sum_grad_epoch_abs))
    ...
    layer_ratio_list = (layer_ratio_list - torch.min(layer_ratio_list)) / (torch.max(layer_ratio_list)-torch.min(layer_ratio_list))
    layer_ratio_list = layer_ratio_list.tolist()
    # print("Epoch: %s, calculated layer_ratio_list: %s" % (epoch, layer_ratio_list))
