...
from utils_cifar100 import *
from copy import deepcopy
from validation_cifar100 import validate, validate_pgd
from torch.utils.tensorboard import SummaryWriter
writer = SummaryWriter('./writer/LHFAT_CIFAR100/')

cudnn.benchmark = True

def parse_args():
    parser = argparse.ArgumentParser(description='PyTorch CIFAR100 Adv Training')
    parser.add_argument('--data', default='./data', metavar='DIR',
                        help='path to dataset')
    parser.add_argument('--output_prefix', default='LHFAT_CIFAR100', type=str,
    ...
    parser.add_argument('--save_dir', default="trained_models/LHFAT/LHFAT_CIFAR100", help='save path')
...
def main():
    ...
    if configs.TRAIN.arch == 'wideresnet_34':
        ...
        model = WRN(depth=34, num_classes=100, epochs=configs.TRAIN.epochs, t_0=configs.t_0, widen_factor=10,
    ...
    for epoch in range(configs.TRAIN.start_epoch, configs.TRAIN.epochs):
        ...
        if epoch%configs.eval_adv_freq==0:
            if epoch>-1:
                ...
                print("eval pgd steps: %d, step_size: %.8f" %(pgd_steps,step_size))
...
def train(train_loader, model, criterion, optimizer, epoch, n_layer):
    ...
    mean = torch.Tensor(np.array(configs.TRAIN.CIFAR100_mean)[:, np.newaxis, np.newaxis])
    mean = mean.expand(3, configs.DATA.crop_size, configs.DATA.crop_size).cuda()
    std = torch.Tensor(np.array(configs.TRAIN.CIFAR100_std)[:, np.newaxis, np.newaxis])
    ...
    #　Reset scheduler
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, iters_per_epoch, eta_min=1e-3*lr_coefficient)
